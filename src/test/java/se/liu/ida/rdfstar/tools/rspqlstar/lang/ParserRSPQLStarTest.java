 ...
 */
public class ParserRSPQLStarTest {
	...
	public void registrationOK() {
		assertTrue( SPARQLParserRegistry.containsParserFactory(RSPQLStar.syntax) );
		assertTrue( SPARQLParserRegistry.parser(RSPQLStar.syntax) instanceof ParserRSPQLStar );

		final String queryString = "" +
				"PREFIX : <http://best_ever#> " +
				"REGISTER STREAM <http://stream> COMPUTED EVERY PT10S " +
				"AS " +
				"SELECT * " +
				"FROM NAMED WINDOW :w1 ON <http://best_ever#s1> [RANGE PT10S STEP PT1S] " +
				"WHERE {" +
				"   GRAPH ?g { ?a ?b ?c } " +
				"   WINDOW :w1 { GRAPH ?g { ?a ?b ?c } }" +
				"}";
		Query query = QueryFactory.create(queryString, RSPQLStar.syntax);
	...
	protected ElementGroup  getElementGroup( String queryString ) {
		final SPARQLParser parser = new ParserRSPQLStar();
