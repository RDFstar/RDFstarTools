...
// Here begins the part of the file that I have extended. -Olaf

        // ---- IRI or marker for begin of embedded triple
        if ( ch == CH_LT ) {
            reader.readChar();
            ...
            if ( ch2 != CH_LT ) {
            	...
            	try {
            		...
            		error("Embedded triple not closed.");
            		tokenS = null;
            	}
            	...
            	if (    tokenS.getType() == TokenType.NODE
               	     && tokenS.getImage().equals(">>") )
            		error("Too few elements in embedded triple.");

            	if (    tokenP.getType() == TokenType.NODE
               	     && tokenP.getImage().equals(">>") )
            		error("Too few elements in embedded triple.");

            	if (    tokenO.getType() == TokenType.NODE
               	     && tokenO.getImage().equals(">>") )
            		error("Too few elements in embedded triple.");
            	...
            	if (    endToken.getType() != TokenType.NODE
            	     || ! endToken.getImage().equals(">>") )
            		error("Marker for end of embedded triple missing.");

            	token = safe;
            	EmbeddedTripleTokenUtils.initializeEmbeddedTripleToken(token, tokenS, tokenP, tokenO);
            }

            return token;
        }

        // ---- possibly marker for end of embedded triple
        if ( ch == CH_GT ) {
            reader.readChar();
            int ch2 = reader.peekChar();
            if ( ch2 != CH_GT ) {
            ...
            {
                reader.readChar();
            	token.setType(TokenType.NODE);
            	token.setImage(">>");
            	return token;
            }
        }

// Here is the end of my extensions. -Olaf
